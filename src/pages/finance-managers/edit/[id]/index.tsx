...
import { getFinanceManagerById, updateFinanceManagerById } from 'apiSdk/finance-managers';
...
function FinanceManagerEditPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<FinanceManagerInterface>(
    () => (id ? `/finance-managers/${id}` : null),
    () => getFinanceManagerById(id),
  );
  ...
  const handleSubmit = async (values: FinanceManagerInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await updateFinanceManagerById(id, values);
      mutate(updated);
      resetForm();
