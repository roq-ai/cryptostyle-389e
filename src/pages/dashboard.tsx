...
import { getPrepaidCards } from 'apiSdk/prepaid-cards';
import { PrepaidCardInterface } from 'interfaces/prepaid-card';
import { getAdmins } from 'apiSdk/admins';
import { AdminInterface } from 'interfaces/admin';
import { getFinanceManagers } from 'apiSdk/finance-managers';
...
function DashboardPage() {
  ...
  const prepaidCardStat = useSWR<PaginatedInterface<PrepaidCardInterface>>(
    () => `/prepaid-cards`,
    () => getPrepaidCards({ limit: 1, offset: 0 }),
  );
  ...
  const financeManagerStat = useSWR<PaginatedInterface<FinanceManagerInterface>>(
    () => `/finance-managers`,
    () => getFinanceManagers({ limit: 1, offset: 0 }),
  );

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Prepaid Cards',
      result: prepaidCardStat?.data?.totalCount,
      isLoading: prepaidCardStat.isLoading,
    ...
    },
    {
      ...
      result: financeManagerStat?.data?.totalCount,
      isLoading: financeManagerStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
