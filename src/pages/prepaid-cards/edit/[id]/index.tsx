...
import { getPrepaidCardById, updatePrepaidCardById } from 'apiSdk/prepaid-cards';
...
function PrepaidCardEditPage() {
  ...
  const handleSubmit = async (values: PrepaidCardInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await updatePrepaidCardById(id, values);
      mutate(updated);
      resetForm();
